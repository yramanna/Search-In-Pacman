...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # Copy the 'center' point's corner visited list; This is a deep copy since cornerVisited is a variable
            ...
            if not isWall:
                ...
                # Check if one of the corners is visited
                for corner in self.corners:
    ...
    def getCostOfActions(self, actions):
        ...
        for action in actions:
            ...
            cost += self.costFn(x,y) # Add the cost of this new position
        return cost

def closestEuclideanPath(fromPosition, positions):
    if len(positions) == 0:
        return 0
    # Get the closest position and the euclidean path distance to it
    closest_position, closest_distance = closest_positionosition(fromPosition, positions)
    # Return the distance + the distance to the rest
    positions.remove(closest_position)
    return closest_distance + closestEuclideanPath(closest_position, positions)

def closestPosition(fromPosition, positions):
    if len(positions) == 0:
        return None

    def fn(input):
    # Euclidean distance is returned as the priority
        position, distance = input
        return distance

    # We use PriorityQueueWithFunction to get easy access to the position with least cost
    ...
    # Get the Euclidean distance to the position at the top of the PriorityQueueWithFunction
    return options.pop()
...
def cornersHeuristic(state, problem):
    ...
    for i in range(4): # There remains at most 4 corners unvisited, so we need to loop four times
        ...
        for cornerDiatance in cornersDistance:
            ...
            # Check each corner: 1. Not visited 2. Distance is lesser than the current closest corner
            # Set it as the new closest corner
...
class FoodSearchProblem:
    ...
    def getCostOfActions(self, actions):
        ...
        cost = 0
        for action in actions:
            # Find the next state and determine if it is legal
            dx, dy = Actions.directionToVector(action)
            x, y = int(x + dx), int(y + dy)
            if self.walls[x][y]:
                return 999999
...
def foodHeuristic(state, problem):
    ...
    H = 0
    maxDistance = 0
    # Find the distance to the farthest food point using A* search, using the mazeDistance() function.
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        closest_position, closest_distance = closestPosition(startPosition, foodPositions)

        return startPosition == closest_position
