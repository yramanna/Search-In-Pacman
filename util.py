...
def matrixAsist( matrix, value = True ):
    """
...
#
# FIXME
# NOTE: TimeoutFuncton is NOT reentrant.  ater timeouts will silently
...
class TimeoutFunction:
    ...
    def __call__(self, *args, **keyArgs):
        ...
        if hasattr(signal, 'SIGARM'):
            old = signal.signal(signal.SIGARM, self.handle_timeout)
            signal.alarm(self.timeout)
            try:
                result = self.function(*args, **keyArgs)
            finally:
                signal.signal(signal.SIGARM, old)
            signal.alarm(0)
        else:
            startTime = time.time()
            result = self.function(*args, **keyArgs)
